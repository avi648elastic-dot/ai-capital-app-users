  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/onboarding/generate-portfolio`, {
        ...
        timeout: 60000 // 60 seconds - should be instant now
      });
      ...
      {loading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-full max-w-sm text-center shadow-xl">
            ...
            <p className="text-slate-400 text-sm">Creating your personalized AI portfolio with optimal stock selections.</p>
          </div>
        </div>
      )}
