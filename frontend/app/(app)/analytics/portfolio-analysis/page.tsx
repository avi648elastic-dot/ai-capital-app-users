...
import { TrendingUp, BarChart3, PieChart, Activity, Building2, TrendingDown, Bot, Smile, Frown, AlertCircle, HelpCircle } from 'lucide-react';
    ...
    <div className="w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
          ...
          <div className="card p-4 sm:p-6">
            ...
            <div className="space-y-4">
              {(sectorData || []).filter(sector => sector.sector !== 'Other' && sector.sector !== 'Unknown').map((sector, index) => (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 bg-slate-800 rounded-lg">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    <span className={`text-2xl font-bold ${
                      (portfolioAnalysis?.performance30D || portfolioAnalysis?.performance90D || 0) >= 0 ? 'text-green-400' : 'text-red-400'
                    }`}>
                    ...
                    <div className="group relative">
                      ...
                      <div className="hidden group-hover:block absolute left-0 bottom-full mb-2 w-56 p-2 bg-slate-900 border border-slate-700 rounded-lg text-xs text-slate-300 z-10 shadow-xl">
                        Real 30-day return calculated from actual price history using metrics engine
                      </div>
                    </div>
                  </div>
                  <div className="text-sm text-slate-400">30 Day Return</div>
                  {(portfolioAnalysis?.performance30D || portfolioAnalysis?.performance90D || 0) >= 5 && (
                    <div className="text-xs text-green-400 mt-1">✨ Excellent</div>
                  )}
                  {(portfolioAnalysis?.performance30D || portfolioAnalysis?.performance90D || 0) < -5 && (
                    <div className="text-xs text-red-400 mt-1">⚠️ Underperforming</div>
                  )}
                </div>
                <div className="text-center p-4 bg-slate-800 rounded-lg">
                ...
                <div className="text-center p-4 bg-slate-800 rounded-lg">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    <span className={`text-2xl font-bold ${
                      (portfolioAnalysis?.portfolioVolatility30D || riskAssessment?.avgVolatility || 0) > 25 ? 'text-red-400' : 
                      (portfolioAnalysis?.portfolioVolatility30D || riskAssessment?.avgVolatility || 0) > 15 ? 'text-yellow-400' : 'text-green-400'
                    }`}>
                      {(portfolioAnalysis?.portfolioVolatility30D || riskAssessment?.avgVolatility || 0).toFixed(1)}%
                    </span>
                    <div className="group relative">
                      <HelpCircle className="w-3.5 h-3.5 text-slate-500 cursor-help" />
                      <div className="hidden group-hover:block absolute left-0 bottom-full mb-2 w-64 p-2 bg-slate-900 border border-slate-700 rounded-lg text-xs text-slate-300 z-10 shadow-xl">
                        Annualized 30-day volatility calculated from daily log returns
                      </div>
                    </div>
                  </div>
                  <div className="text-sm text-slate-400">Volatility (30D)</div>
                  {(portfolioAnalysis?.portfolioVolatility30D || riskAssessment?.avgVolatility || 0) < 15 && (
                    <div className="text-xs text-green-400 mt-1">Low</div>
                  )}
                  {(portfolioAnalysis?.portfolioVolatility30D || riskAssessment?.avgVolatility || 0) > 25 && (
          ...
          <div className="lg:col-span-2 card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {sectorPerformance && sectorPerformance.length > 0 ? (
                sectorPerformance.filter(sector => sector.sector !== 'Other' && sector.sector !== 'Unknown').map((sector, index) => (
